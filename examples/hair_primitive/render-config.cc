...
bool LoadRenderConfig(example::RenderConfig* config, const char* filename) {
  ...
  if (o.find("cyhair_filename") != o.end()) {
    if (o["cyhair_filename"].is<std::string>()) {
      config->cyhair_filename = o["cyhair_filename"].get<std::string>();
    }
  }
  ...
  config->max_strands = -1;
  if (o.find("max_strands") != o.end()) {
    if (o["max_strands"].is<double>()) {
      config->max_strands = static_cast<int>(o["max_strands"].get<double>());
    }
  }
