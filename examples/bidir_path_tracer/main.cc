...
typedef nanort::BVHAccel<nanort::TriangleMesh, nanort::TriangleSAHPred,
...
class Random {
 public:
  ...
  unsigned int nextInt(int n) { return nextInt() % n; }

  float nextReal() { return (float)nextInt() / (float)UINT_MAX; }
...
enum VertexType { Light, Lens, Surface };
...
struct Vertex {
  ...
  float3 f(const Vertex &v) const {
    ...
    float rhoD = vdot(float3(1, 1, 1) / 3.0f, diffuseColor) * (1.0 - fresnel) *
                 (1.0 - mat.dissolve);
...
class LightSampler {
 public:
  ...
  int sample(const Mesh &mesh, Random &rng, float3 *pos, float3 *norm,
             float *pdf) const {
    // Sample light with binary search.
    float rand = rng.nextReal();
    int sampleId =
        std::lower_bound(cdf_.begin(), cdf_.end(), rand) - cdf_.begin();
...
float3 sampleBRDF(const tinyobj::material_t &mat, const float3 &wo,
                  const float3 &origNorm, const float3 &norm, Random &rng,
                  float3 *wi, float *pdf) {
  float3 diffuseColor(mat.diffuse);
  float3 specularColor(mat.specular);
  float3 refractionColor(mat.transmittance);
  ...
  float rhoD = vdot(float3(1, 1, 1) / 3.0f, diffuseColor) * (1.0 - fresnel) *
               (1.0 - mat.dissolve);
  float rhoR = vdot(float3(1, 1, 1) / 3.0f, refractionColor) * (1.0 - fresnel) *
               mat.dissolve;
...
void raytrace(const Mesh &mesh, const Accel &accel,
              const std::vector<tinyobj::material_t> &materials, Random &rng,
              float3 rayOrg, float3 rayDir, float3 beta, float pdf,
  ...
  for (int b = 0; b < uMaxBounces; ++b) {
    ...
    float3 outDir;
    float3 f =
        sampleBRDF(mat, -rayDir, originalNorm, norm, rng, &outDir, &pdfFwd);
...
void eyeSubpath(int x, int y, int width, int height, const Mesh &mesh,
                const std::vector<tinyobj::material_t> &materials,
  ...
  float3 rayDir =
      float3((px / (float)width) - 0.5f, (py / (float)height) - 0.5f, -1.0f);
  rayDir.normalize();
  ...
  raytrace(mesh, accel, materials, rng, rayOrg, rayDir, beta, pdf, eyeVert,
           true);
}

void lightSubpath(const Mesh &mesh,
                  const std::vector<tinyobj::material_t> &materials,
                  const Accel &accel, Random &rng, const LightSampler &lights,
  ...
  float pdfDir;
  float3 rayDir =
      directionCosTheta(norm, rng.nextReal(), rng.nextReal(), &pdfDir);
  ...
  raytrace(mesh, accel, materials, rng, rayOrg, rayDir, beta, pdf, lightVert,
           false);
}

float weightMIS(const LightSampler &light, const std::vector<Vertex> &eyeVert,
                const std::vector<Vertex> &lightVert, int numEyeVert,
                int numLightVert, const Mesh &mesh, const Accel &accel) {
  ...
  const Vertex *vl =
      numLightVert - 1 >= 0 ? &lightVert[numLightVert - 1] : nullptr;
  const Vertex *veMinus =
      numEyeVert - 2 >= 0 ? &eyeVert[numEyeVert - 2] : nullptr;
  const Vertex *vlMinus =
      numLightVert - 2 >= 0 ? &lightVert[numLightVert - 2] : nullptr;
  ...
  if (ve) {
    if (numLightVert == 0) {
      ...
      path[numEyeVert - 1].second =
          pdfOmega * std::abs(vdot(vl->norm, wo)) / (dist * dist);
    }
  }
  ...
  if (vl) {
    if (numEyeVert <= 1) {
      ...
      path[numEyeVert].second =
          pdfOmega * std::abs(vdot(ve->norm, wo)) / (dist * dist);
    }
  }
  ...
  for (int i = numEyeVert; i < length; i++) {
    ...
    if (lightVert[length - i - 1].isDelta() ||
        (i + 1 < length && lightVert[length - i - 2].isDelta()))
      continue;
    mis += prob * prob;
  }
...
float calcG(const Vertex &v1, const Vertex &v2, const Mesh &mesh,
            const Accel &accel) {
...
float3 connectPath(const std::vector<Vertex> &eyeVert,
                   const std::vector<Vertex> &lightVert, const Mesh &mesh,
                   const Accel &accel, const LightSampler &light) {
  float3 color(0.0f, 0.0f, 0.0f);
  if (eyeVert[eyeVert.size() - 1].type == Light) {
    float mis =
        weightMIS(light, eyeVert, lightVert, eyeVert.size(), 0, mesh, accel);
