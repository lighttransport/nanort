...
static const int uMaxBounces = 6;
static const int SPP = 8;
...
#include <ctime>
    ...
    public:
        ...
        int sample(const Mesh &mesh, float3 *pos, float3 *norm, float *pdf) const {
            // Sample light with binary search.
            float r = rand() / (float)RAND_MAX;
            int sampleId = std::lower_bound(cdf_.begin(), cdf_.end(), r) - cdf_.begin();
...
void raytrace(const Mesh &mesh, const Accel &accel, const std::vector<tinyobj::material_t> &materials,
    ...
    for (int b = 0; b < uMaxBounces; ++b) {
        ...
        else
        {
            ...
            if (isEyePath) {
                refl = float3(0.0f, 0.0f, 0.0f);
                beta = beta * emissiveColor;
                pdfOmega = 1.0f;
                vtype = Light;
            } else {
                break;
...
void eyeSubpath(int x, int y, int width, int height,
    ...
    raytrace(mesh, accel, materials, rayOrg, rayDir, beta, pdf, eyeVert, true);
}

void lightSubpath(const Mesh &mesh, const std::vector<tinyobj::material_t> &materials, const Accel &accel,
                  const LightSampler &lights, std::vector<Vertex> *lightVert) {
    // Clear vertices.
    lightVert->clear();
    ...
    float3 rayOrg, norm;
    float pdfPos;
    int lightId = lights.sample(mesh, &rayOrg, &norm, &pdfPos);
    ...
    raytrace(mesh, accel, materials, rayOrg, rayDir, beta, pdf, lightVert, false);
}
...
float pdfAt(const std::vector<const Vertex*> &path, int prev, int curr, int next) {
    ...
    switch (path[curr]->type) {
        case Diffuse:
            pdf = vdot(wi, path[curr]->normal) / M_PI;
...
float weightMIS(const std::vector<Vertex> &eyeVert, const std::vector<Vertex> &lightVert,
    ...
    if (eyeId + lightId == 1) return 1.0f;
    ...
    for (int i = eyeId; i >= 1; i--) {
        ...
        float pdfFwd = pdfAt(path, i - 2, i - 1, i);
        prob *= pdfRev / pdfFwd;
        
        if (path[i]->type == Specular ||
            (i - 1 >= 0 && path[i - 1]->type == Specular)) continue;
        mis += prob * prob;
    }
    ...
    for (int i = eyeId + 1; i < pathLen; i++) {
        ...
        if (path[i]->type == Specular ||
            (i + 1 < pathLen && path[i + 1]->type == Specular)) continue;
        mis += prob * prob;
    }
...
float3 connectPath(const std::vector<Vertex> &eyeVert, const std::vector<Vertex> &lightVert,
    ...
    for (int e = 1; e < eyeVert.size(); e++) {
        const Vertex &ev = eyeVert[e];
        if (e == eyeVert.size() - 1 && ev.type == Light) {
            //color += ev.beta / ev.pdf;
            continue;
        }
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            for(int i = 0; i < SPP; ++i) {
                ...
                std::vector<Vertex> eyeVert;
                eyeSubpath(x, y, width, height, mesh, materials, accel, &eyeVert);
                
                std::vector<Vertex> lightVert;
                lightSubpath(mesh, materials, accel, lights, &lightVert);
