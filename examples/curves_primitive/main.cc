...
class CurveGeometry {
 public:
  ...
  void BoundingBox(float3 *bmin, float3 *bmax, unsigned int prim_index) const {
    ...
    (*bmax)[0] = vertices_[3 * (4 * prim_index + 0) + 0] + radiuss_[4 * prim_index];
    (*bmax)[1] = vertices_[3 * (4 * prim_index + 0) + 1] + radiuss_[4 * prim_index];
    (*bmax)[2] = vertices_[3 * (4 * prim_index + 0) + 2] + radiuss_[4 * prim_index];
    for (int i = 1; i < 4; i++) {
      (*bmin)[0] = std::min(vertices_[3 * (4 * prim_index + i) + 0] - radiuss_[4 * prim_index + i], (*bmin)[0]);
      (*bmin)[1] = std::min(vertices_[3 * (4 * prim_index + i) + 1] - radiuss_[4 * prim_index + i], (*bmin)[1]);
      (*bmin)[2] = std::min(vertices_[3 * (4 * prim_index + i) + 2] - radiuss_[4 * prim_index + i], (*bmin)[2]);
      (*bmax)[0] = std::max(vertices_[3 * (4 * prim_index + i) + 0] + radiuss_[4 * prim_index + i], (*bmax)[0]);
      (*bmax)[1] = std::max(vertices_[3 * (4 * prim_index + i) + 1] + radiuss_[4 * prim_index + i], (*bmax)[1]);
      (*bmax)[2] = std::max(vertices_[3 * (4 * prim_index + i) + 2] + radiuss_[4 * prim_index + i], (*bmax)[2]);
    }
  }
...
class CurveIntersector
{
  ...
  // See "Exploiting Local Orientation Similarity for Efficient Ray Traversal of Hair and Fur" http://www.sven-woop.de/
  // And Embree implementation for more details.
    
 public:
  CurveIntersector(const float *vertices, const float *radiuss, const int num_subdivisions = 4)
      : vertices_(vertices), radiuss_(radiuss), num_subdivisions_(num_subdivisions) {}
