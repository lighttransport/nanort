...
bool LoadRenderConfig(example::RenderConfig* config, const char* filename) {
  ...
  if (o.find("partio_filename") != o.end()) {
    if (o["partio_filename"].is<std::string>()) {
      config->partio_filename = o["partio_filename"].get<std::string>();
    }
  }
  ...
  config->constant_radius = 1.0f;
  if (o.find("constant_radius") != o.end()) {
    if (o["constant_radius"].is<double>()) {
      config->constant_radius = static_cast<float>(o["constant_radius"].get<double>());
    }
  }
